...
def assess_usage_with_ai(water_gallons, electricity_kwh, gas_cubic_m, user=None):
    ...
    if eco_ai.is_trained and len(data_for_analysis) > 0:
        ...
        # Generate basic recommendations when no training data
        ai_recommendations = eco_ai.generate_recommendations(water_gallons, electricity_kwh, gas_cubic_m)
...
def calculate_ai_points(water_status, electricity_status, gas_status, efficiency_score, user=None):
    """Enhanced AI-based points calculation with smart scoring and maximum reward parameters"""
    # Enhanced status mapping with more granular scoring
    status_values = {
        'Excellent': 10,
        'Very Good': 9,
        'Good': 8,
    ...
    # Get base points for each utility with fallback handling
    water_points = status_values.get(water_status, 5)
    electricity_points = status_values.get(electricity_status, 5)
    gas_points = status_values.get(gas_status, 5)
    
    # Calculate weighted average (water has slightly higher weight due to scarcity)
    weighted_average = (water_points * 1.1 + electricity_points * 1.0 + gas_points * 0.9) / 3.0
    
    # Smart efficiency modifier with progressive scaling
    if efficiency_score >= 90:
        efficiency_modifier = 1.25  # Maximum bonus for exceptional efficiency
    elif efficiency_score >= 80:
        efficiency_modifier = 1.15
    elif efficiency_score >= 70:
        efficiency_modifier = 1.05
    elif efficiency_score >= 50:
        efficiency_modifier = 1.0   # Neutral
    elif efficiency_score >= 30:
        efficiency_modifier = 0.9
    else:
        efficiency_modifier = 0.8   # Penalty for poor efficiency
    
    # Advanced context modifiers with maximum reward parameters
    context_modifier = 1.0
    max_context_bonus = 0.3  # Maximum 30% bonus from context
    
    if user:
        # Household efficiency bonus (points per person efficiency)
        household_size = getattr(user, 'household_size', 1)
        if household_size > 1:
            # Reward efficiency per person - larger households get bonus for good per-capita usage
            per_person_bonus = min(0.05 * (household_size - 1), 0.15)  # Max 15% bonus
            context_modifier += per_person_bonus
        
        # Energy features smart bonus system
        ...
        if energy_features:
            try:
                ...
                # Progressive bonus system with diminishing returns
                if feature_count >= 10:
                    features_bonus = 0.20  # Maximum 20% for comprehensive green setup
                ...
                # Bonus for specific high-impact features
                high_impact_features = ['Solar', 'solar', 'Geothermal', 'geothermal', 'Heat Pump', 'Battery Storage']
                high_impact_count = sum(1 for feature in features if any(impact in feature for impact in high_impact_features))
                if high_impact_count > 0:
                    features_bonus += min(0.05 * high_impact_count, 0.10)  # Max 10% additional
                
                context_modifier += features_bonus
            except:
                pass
        
        # Location-based adjustments
        location_type = getattr(user, 'location_type', '')
        if 'Rural' in location_type:
            context_modifier += 0.05  # Rural areas often have higher baseline usage
        elif 'Urban - Large City' in location_type:
            context_modifier += 0.03  # Urban efficiency bonus
        
        # Climate zone considerations
        climate_zone = getattr(user, 'climate_zone', '')
        if any(climate in climate_zone for climate in ['Polar', 'Continental']):
            context_modifier += 0.05  # Cold climate adjustment
        elif any(climate in climate_zone for climate in ['Tropical', 'Arid']):
            context_modifier += 0.03  # Hot climate adjustment
    
    # Apply maximum context bonus cap
    context_modifier = min(context_modifier, 1.0 + max_context_bonus)
    
    # Calculate final AI points with smart scaling
    base_score = weighted_average * efficiency_modifier * context_modifier
    
    # Apply progressive scaling to reward exceptional performance
    if base_score >= 9:
        final_points = 9 + (base_score - 9) * 2  # Amplify top performance
    elif base_score >= 7:
        final_points = base_score * 1.1  # Slight boost for good performance
    else:
        final_points = base_score  # No modification for average/poor performance
    
    # Ensure bounds and apply maximum reward cap
...
reset_code = st.sidebar.text_input(
    "🔑 Reset Code:", 
    type="password",
    placeholder="Atishay,Akshaj,Adit@EcoAudit_Team",
    help="Users are permanently protected. Only creator team can reset with: Atishay,Akshaj,Adit@EcoAudit_Team",
    key="reset_code_input"
)

if st.sidebar.button("🔥 RESET APPLICATION", type="secondary", help="This will delete ALL data"):
    if reset_code == "Atishay,Akshaj,Adit@EcoAudit_Team":
...
if page == "User Profile":
    ...
    if st.session_state.current_user is None:
        ...
        with tab2:
            ...
            confirmation_code = st.text_input(
                "Create Security Code (5-10 digits)", 
                type="password",
                key="new_confirmation_code",
                help="Create a private 5-10 digit code that only you will know. This code is required for login and account access.",
                max_chars=10
            )
    ...
    **Note:** Utility usage tracking is limited to 2 entries per day per user. Materials recycling guide searches remain unlimited.
    ...
    if assess_button or save_button:
        ...
        water_status, electricity_status, gas_status, ai_analysis = assess_usage_with_ai(water, electricity, gas, current_user)
        ...
        if save_button:
            with st.spinner("Checking daily limits and saving data..."):
                ...
                if not can_save:
                    ...
                    # Enforce maximum 20 points limit for the day
                    if final_points > 10:  # Cap single entry at 10 points (2 entries max = 20 points total)
                        final_points = 10
                    ...
                    if success:
                        ...
                        # Update user's total points with the final calculated points
                        db.update_user_points(current_user.id, final_points)
        ...
        # Lazy load chart creation for better performance
        if st.button("Show Usage Chart", help="Click to display usage visualization"):
