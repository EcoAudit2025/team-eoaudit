...
from intelligent_chatbot import IntelligentEcoBot
import numpy as np
import translations
...
if not st.session_state.auto_login_checked and st.session_state.last_username and not st.session_state.current_user:
    try:
        # Try to restore user session if they were logged in before
        if st.session_state.remember_login:
            user = db.get_user(st.session_state.last_username, is_public='private')
            if not user:
                user = db.get_user(st.session_state.last_username, is_public='public')
            if user:
        ...
        if save_button:
            with st.spinner("Checking daily limits and saving data..."):
                ...
                if not can_save:
                    ...
                    # Save to database with user ID and get result
                    ...
                    if success:
                        ...
                        # Calculate and update sustainability points
                        points = db.calculate_sustainability_points(water, electricity, gas)
                        db.update_user_points(current_user.id, points)
