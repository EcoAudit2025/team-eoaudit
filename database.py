...
def check_daily_usage_limit(user_id):
    """Check if user has reached daily usage limit (2 entries per day)"""
    session = get_session()
    try:
        user = session.query(User).filter(User.id == user_id).first()
        if not user:
        ...
        if user.daily_usage_count >= 2:
            ...
            return False, f"Daily limit reached. Next entry available in {hours_left}h {minutes_left}m"
        
        return True, f"You can save {2 - user.daily_usage_count} more entries today"
...
def save_utility_usage(user_id, water_gallons, electricity_kwh, gas_cubic_m, water_status, electricity_status, gas_status, efficiency_score=None, carbon_footprint=None):
    """Save utility usage data to the database for a specific user with daily limit check"""
    session = get_session()
    try:
