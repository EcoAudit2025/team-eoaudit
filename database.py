...
def calculate_environmental_class(user_usage_data):
    ...
    # Define thresholds (can be adjusted based on regional standards)
    water_threshold_excellent = 8000  # gallons/month
    water_threshold_good = 12000
    
    electricity_threshold_excellent = 500  # kWh/month
    electricity_threshold_good = 800
    
    gas_threshold_excellent = 80  # cubic meters/month
    gas_threshold_good = 120
    
    # Calculate score (lower is better)
    score = 0
    
    # Water scoring
    if avg_water <= water_threshold_excellent:
        score += 1
    elif avg_water <= water_threshold_good:
        score += 2
    else:
        score += 3
    
    # Electricity scoring
    if avg_electricity <= electricity_threshold_excellent:
        score += 1
    elif avg_electricity <= electricity_threshold_good:
        score += 2
    else:
        score += 3
    
    # Gas scoring
    if avg_gas <= gas_threshold_excellent:
        score += 1
    elif avg_gas <= gas_threshold_good:
        score += 2
    else:
        score += 3
    
    # Determine class based on total score
    if score <= 4:
        return 'A'  # Excellent environmental impact
    elif score <= 7:
        return 'B'  # Good environmental impact
    else:
...
def generate_user_ai_analysis(user, usage_data):
    ...
    # Overall assessment
    if env_class == 'A':
        analysis_parts.append("Excellent environmental steward! Your utility usage is well below average and demonstrates strong conservation habits.")
    elif env_class == 'B':
        analysis_parts.append("Good environmental awareness. Your usage patterns show conscious effort toward sustainability with room for optimization.")
    else:
        analysis_parts.append("Significant opportunity for improvement. Your current usage patterns have a higher environmental impact.")
...
def initialize_materials():
    ...
    if existing_users == 0:
        test_user = User(
            ...
            location_type='Urban - Large City (>1M population)',
            climate_zone='Temperate - Moderate Seasons',
            adults=2,
            children=1,
            seniors=0,
            household_type='Family with Young Children',
            housing_type='Apartment - 2 Bedroom',
            household_size=3,
            is_public='public',
            language='English'
        )
        ...
        # Add some sample usage data for the test user
        from datetime import datetime, timedelta
        import random
        ...
        for i in range(5):
            ...
            usage = UtilityUsage(
                ...
                timestamp=date,
                water_gallons=random.randint(3000, 8000),
                electricity_kwh=random.randint(300, 700),
                gas_cubic_m=random.randint(50, 120),
                water_status='Normal',
                electricity_status='Normal',
                gas_status='Normal',
                efficiency_score=random.uniform(70, 95),
                carbon_footprint=random.uniform(500, 1200)
            )
            session.add(usage)
        
        session.commit()
        print("Test user created with sample data")
