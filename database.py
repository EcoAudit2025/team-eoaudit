...
class User(Base):
    ...
    confirmation_code = Column(String(20))  # Private 5-20 digit confirmation code for security
...
def validate_confirmation_code(code):
    """Validate confirmation code format (5-20 digits)"""
    if not code:
    ...
    if len(code) < 5 or len(code) > 20:
        return False, "Confirmation code must be between 5-20 digits"
...
def get_time_until_reset(user_id):
    """Get time remaining until daily usage limit resets based on IST timezone"""
    session = get_session()
    try:
        user = session.query(User).filter(User.id == user_id).first()
        if not user:
        ...
        # Get most recent entries to check 24-hour window
        recent_entries = session.query(UtilityUsage).filter(
            UtilityUsage.user_id == user_id
        ).order_by(UtilityUsage.timestamp.desc()).limit(2).all()
        
        if len(recent_entries) >= 2:
            # Calculate 24-hour window from second most recent entry
            second_recent_entry = recent_entries[1]
            entry_time = second_recent_entry.timestamp
            ...
            if current_time_utc < next_available_time:
                ...
                return f"{hours:02d}:{minutes:02d} (24h timer)"
        
        # Fallback to IST midnight reset
        if not user.last_usage_date:
            return "00:00:00"
        ...
        return f"{hours:02d}:{minutes:02d} (IST)"
...
def save_utility_usage(user_id, water_gallons, electricity_kwh, gas_cubic_m, water_status, electricity_status, gas_status, efficiency_score=None, carbon_footprint=None, ai_points=None):
    ...
    try:
        ...
        # Update user's daily usage count and last usage date using IST
        user = session.query(User).filter(User.id == user_id).first()
        if user:
            ist = pytz.timezone('Asia/Kolkata')
            current_time_utc = datetime.now(pytz.UTC)
            ...
            user.last_usage_date = current_time_utc.replace(tzinfo=None)  # Store as UTC naive
...
def delete_user_account_permanently(user_id, admin_code=None):
    """Delete a user account and all associated data permanently
    NOTE: This function is disabled for data protection. Users are saved permanently.
    Only the creator can reset the entire application with the proper reset code.
    """
    # Data protection - prevent individual user deletion
    return False, "User deletion disabled for data protection. Users are saved permanently until application reset by creator."

def reset_entire_application(reset_code):
    """CREATOR'S ONLY: Reset the entire application - DELETE ALL DATA
    Requires special reset code for security (confidential)
    """
    # Security check - only allow reset with correct code
    VALID_RESET_CODE = "Atishay,Adit,Akshaj@EcoAudit_Team"
    
    if reset_code != VALID_RESET_CODE:
        return False, "Invalid reset code. Data protection active - users are safe."
    ...
    try:
        ...
        return True, f"Application successfully reset with code verification! All users and data have been deleted. Reset performed at {datetime.now()}."
    except Exception as e:
        session.rollback()
        session.close()
